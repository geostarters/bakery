...
import urllib2, io, math, cStringIO, os, sys
...
def work():
	global zoomLevel, tileX, tileY, numDone
...
def getTileIndex():
	global numDone, tileZoom, tileX, tileY, levelTiles, startZoom
	numDone = numDone+1
	tileArrayIndex = tileZoom - startZoom
	if(len(levelTiles) > tileArrayIndex):
		indexes = (tileZoom, tileX, tileY)
		(top, left, bottom, right) = levelTiles[tileArrayIndex]
		if(tileX < right):
			tileX = tileX+1
		else:
			tileX = left
			...
			if(tileY > bottom):
				tileZoom = tileZoom+1
				tileArrayIndex = tileArrayIndex + 1
				if(len(levelTiles) > tileArrayIndex):
					print "----", tileZoom, "----"
					(top, left, bottom, right) = levelTiles[tileArrayIndex]
